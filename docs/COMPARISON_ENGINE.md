...
Status: implemented (Phase 11), unit-tested, MVP scope.
...
This is the evidence Phase 12 (reproducibility classification) will
consume next. No DB session is touched anywhere in this phase — comparators
...
real DB session belongs to Phase 19 (FastAPI).
...
Every one of the 5 comparators calls
...
Each returns a `<Category>ComparisonResult(status, differences)`. All use
their category's own fingerprint hash as a SAME/DIFFERENT fast path, then
(on DIFFERENT) diff the underlying fields to explain why:
...
## `PARTIALLY_MATCHING` is deliberately unused

`ComparisonStatus.PARTIALLY_MATCHING` exists in the enum but no comparator
in this phase assigns it. Working through all 5 categories, it never has an
...
to Phase 12 (reproducibility classification), which is free to assign
`PARTIALLY_MATCHING` at the run-comparison level.

## Scope deferred to later phases

- **`metrics_status`** is always `ComparisonStatus.UNKNOWN` — real metric
  comparison needs abs/rel tolerance, which is Phase 13's job. No metrics
...
`base_*`/`compare_*` kwargs style, runs all 5 comparators, forces
`metrics_status = UNKNOWN`, and concatenates every category's differences
into one `ComparisonResult`.
